...
***************************************************************************/
void clip_window(t_index r, t_index c, t_pixel *iw, t_pixel *ow) {

    t_test first_row, last_row, first_col, last_col;
...
***************************************************************************/
t_pixel thresholding(t_pixel pixl, t_pixel threshold)  {
...
***************************************************************************/
void buffer(t_pixel din, t_index c, t_pixel *col_window) {
...
***************************************************************************/
int system_input(t_pixel d_in, t_pixel *cliped_window, t_bool *last_pixl) {

    static t_index  c = 0, r = 0;
    static t_index  c_filter = 0, r_filter = 0;
    static t_bool   output_vld = 0;
    static t_pixel  window[9];
    t_pixel         col_window[3];

    /* ulozeni pixelu do bufferu, posun okenka a clipping */
    buffer(d_in, c, col_window);
    ...
    *last_pixl = ((r_filter == FRAME_ROWS-1) && (c_filter == FRAME_COLS-1));
...
void filter(t_pixel &in_data, bool &in_data_vld, t_pixel &out_data, t_mcu_data mcu_data[MCU_SIZE]){
   ...
   if (!mcu_ready) {
      if(mcu_data[FPGA_MCU_READY] == 1) {
    	...
    	t_pixel pix_filtered;
			t_pixel window[9];
    	t_bool output_vld_data;
    	t_bool last_pixel;
    	static t_pixel threshold = 4;
    	static t_frame frame = 1;

    	output_vld_data = system_input(in_data, window, &last_pixel);
    	if (output_vld_data) {
