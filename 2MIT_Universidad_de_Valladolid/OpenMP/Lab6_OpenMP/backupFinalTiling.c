...
int main() {
	...
	// for (k = 1; k < SIZE*2; k++) {
		...
		for (ik = 1; ik < SIZE ; ik+=BLOCKSIZE) {
			for (jk = 1; jk < SIZE; jk+=BLOCKSIZE) {
				...
				// Fase decrecimiento
				int newSIZE = newBLOCKSIZE;
				for(d=ik; d<newSIZE; d++)
					for(e=jk; e<=newSIZE-d; e++) {
						printf("d: %d e: %d newBLOCKSIZE: %d", d,e, newBLOCKSIZE );
						printf("    [%d,%d]\n",newSIZE-e, e+d-1);							
						A[newSIZE-e][e+d-1-(ik-1)] = A[newSIZE-e-1][e+d-1-(ik-1)] + A[newSIZE-e][e+d-2-(ik-1)];					
					}	
	...
	// 		// Fase decrecimiento
	// 		for(d=1; d<SIZE-BLOCKSIZE; d++)
	// 			for(e=1; e<=SIZE-BLOCKSIZE-d; e++) {
	// 				A[SIZE-BLOCKSIZE-e][e+d-1] = A[SIZE-BLOCKSIZE-e-1][e+d-1] + A[SIZE-BLOCKSIZE-e][e+d-2];
	// 				//printf("d: %d e: %d\n",d, e );
	// 			}
	...
	// 		for(d=1; d<=BLOCKSIZE; d++)
	// 			for(e=5; e<=d+BLOCKSIZE; e++) {
